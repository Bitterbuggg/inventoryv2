 ...
 * @internal
 */
final class ReceivingRouteGuardTest extends CIUnitTestCase
{
    ...
    public function testGuestCannotAccessReceivingPage(): void
    {
        auth('session')->logout();
        session()->remove('user');
        session()->destroy();
    ...
    public function testEmployeeCannotAccessReceivingPage(): void
    {
        $employee = $this->findUserByEmail('employee@local.test');
        auth('session')->login($employee);

        $response = $this->withSession(session()->get())->get('/receiving');
        $response->assertStatus(403);
    }

    public function testItStaffCanAccessReceivingPage(): void
    {
